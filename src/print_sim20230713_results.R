...
get_no_sampling_results <- function(){
  ...
  #   SAMPLE_SIZE <- N; b=0
    ...
    no_sampling_results <- readRDS(paste0("outputs/no_sampling_results-PHIX",PHIX,".rds"))
...
compute_bias <- function(rslt_summ, no_sampling_results){
  ...
  for(i in names(rslt_summ)[1:4]){
    bias[[i]]<- rslt_summ[[i]]-no_sampling_results[[i]]
  }
  return(bias[1:4])
}
...
print_results <- function(){
  row_names_set <- c("beta0","beta1","sigma2","phi","variogsigma2","variogphi")
  ...
  print(paste("PHIX=",PHIX," and SAMPLE SIZE=",SAMPLE_SIZE))
    ...
    rslt <- readRDS(
    paste0(
      "outputs/reg-est-loc-2023-07-25-sim3-gevsampsize",
      SAMPLE_SIZE,
      "phi",
      PHIX,
      ".rds"
  ...
  bias <- compute_bias(rslt_summ, no_samp)
